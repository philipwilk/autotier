...
void validate_backend_path(const fs::path &path,
	...
	bool no_perm = false;
	if (path.is_relative()) {
		Logging::log.error(prefix + ": " + path_desc + " must be an absolute path: \""
						   + path.string() + "\"");
		errors = true;
		return;
	}
	try {
		...
		Logging::log.error(prefix + ": Failed to check " + path_desc + ": "
						   + std::string(e.what()));
		errors = true;
		no_perm = true;
	}
	if (!is_directory && !no_perm && create) {
		try {
			...
			Logging::log.error(prefix + ": Failed to create " + path_desc + ": "
							   + std::string(e.what()));
			errors = true;
			no_perm = true;
		}
	...
	if (is_directory && !no_perm) {
		...
		if (access(path.c_str(), mode) != 0) {
			int err = errno;
			Logging::log.error(prefix + ": Cannot access " + path_desc + ": "
							   + std::string(strerror(err)) + ": \"" + path.string() + "\"");
			errors = true;
		}
	}
	if (!is_directory && !no_perm) {
...
Config::Config(const fs::path &config_path,
			   std::list<Tier> &tiers,
			   const ConfigOverrides &config_overrides) try
	: ffd
	::ConfigParser(config_path.string()) {
...
void Config::load_config(const fs::path &config_path,
	...
	if (global_header_itr == valid_global_headers.end()) {
		Logging::log.warning(
			"No global section in config! Trying top level scope or defaults (no tiering).");
		int log_level_tmp = get<int>("Log Level", LogLevel::NORMAL);
		log_level_ =
			(Logger::log_level_t)(log_level_tmp > 2 ? 2 : (log_level_tmp < 0 ? 0 : log_level_tmp));
		copy_buff_sz_ = get<ffd::Bytes>("Copy Buffer Size", ffd::Bytes(1024 * 1024)).get();
		tier_period_s_ =
			std::chrono::seconds(get<int64_t>("Tier Period", int64_t(TIER_PERIOD_DISBLED)));
		strict_period_ = get<bool>("Strict Period", false);
	...
	for (ffd::ConfigNode *subsection : sub_confs_) {
		...
		if (log_level_ >= 2) {
			...
			Logging::log.message("Tier quota: " + std::to_string(quota.get_fraction() * 100.0)
									 + "% " + quota.get_str() + " ("
									 + ffd::Bytes(quota.get_max()).get_str() + ")",
...
void Config::dump(const std::list<Tier> &tiers, std::stringstream &ss) const {
	...
	for (const Tier &t : tiers) {
		...
		ss << "Quota = " << t.quota().get_fraction() * 100.0 << " % (" << t.quota().get_str() << ")"
		   << std::endl;
		ss << " " << std::endl;
	}
}
