...
int get_command_index(const std::string &cmd) {
	std::regex command_list[NUM_COMMANDS] = { std::regex("^[Oo]neshot|ONESHOT$"),
											  std::regex("^[Pp]in|PIN$"),
...
void sanitize_paths(std::list<std::string> &paths) {
	...
	if (!directories.empty()) {
		std::string err_msg =
			"autotier cannot process entire directories. Pass files instead, or use `find <path> -type f | xargs autotier <command>`. Offender(s):";
